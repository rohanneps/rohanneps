...
from project.tasks import start_async_image_match
		...
		if serializer.is_valid():
			...
			if currently_running_project_count != 0:
				...
				if helper.check_input_file_exists(project_id,project.file_name):
					...
					return Response({'response':'Project started for id:{} not started as file hasn\'t been copied from ftp location'.format(project_id),'started':False})
		else:
			comp_logger.info('{} is not valid'.format(data))
			comp_logger.info(serializer.errors)
			return Response({'response':serializer.errors})
